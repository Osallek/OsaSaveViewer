                            ...
                            }
                            {
                              (index === Object.keys(war.attackers).filter(tag => charts[0][tag] > 0).length - 1) && (charts[1].total < charts[0].total) &&
                                <LabelList dataKey={ tag } position='top' fill='red'
                                           formatter={ (value: number) => `+${ formatNumber(charts[0].total - charts[1].total) } (+${ formatNumber((1 - (charts[1].total / charts[0].total)) * 100) }%)` }/>
                            }
                          </Bar>
                        ))
                    }
                          ...
                          <Bar dataKey={ tag } stackId='total' isAnimationActive={ false } key={ `bar-total-${ tag }` }
                            ...
                            }
                            {
                              (index === Object.keys(war.defenders).filter(tag => charts[1][tag] > 0).length - 1) && (charts[0].total < charts[1].total) &&
                                <LabelList dataKey={ tag } position='top' fill='red'
                                           formatter={ (value: number) => `+${ formatNumber(charts[1].total - charts[0].total) } (+${ formatNumber((1 - (charts[0].total / charts[1].total)) * 100) }%)` }/>
                            }
                          </Bar>
                        ))
                    }
                                       ...
                                       formatter={ (value: number) => {
                                         ...
                                         if (index === values.length - 1) {
                                           if (charts[0]['land'] > charts[1]['land'] && value === charts[0][type]) {
                                             return `+${ formatNumber(charts[0]['land'] - charts[1]['land']) } (+${ formatNumber(((charts[0]['land'] / charts[1]['land']) - 1) * 100) }%)`
                                           } else if (charts[1]['land'] > charts[0]['land'] && value === charts[1][type]) {
                                             return `+${ formatNumber(charts[1]['land'] - charts[0]['land']) } (+${ formatNumber(((charts[1]['land'] / charts[0]['land']) - 1) * 100) }%)`
                                           }
                                         }
                                       ...
                                       formatter={ (value: number) => {
                                         ...
                                         if (index === values.length - 1) {
                                           if (charts[0]['naval'] > charts[1]['naval'] && value === charts[0][type]) {
                                             return `+${ formatNumber(charts[0]['naval'] - charts[1]['naval']) } (+${ formatNumber(((charts[0]['naval'] / charts[1]['naval']) - 1) * 100) }%)`
                                           } else if (charts[1]['naval'] > charts[0]['naval'] && value === charts[1][type]) {
                                             return `+${ formatNumber(charts[1]['naval'] - charts[0]['naval']) } (+${ formatNumber(((charts[1]['naval'] / charts[0]['naval']) - 1) * 100) }%)`
                                           }
                                         }
                                     ...
                                     fill={ colorToHex(getCountry(save, tag).colors.countryColor) }>
                                  {
                                    charts[0][`${ tag }-${ type }`] / charts[0][type] > 0.03 &&
                                      <LabelList dataKey={ `${ tag }-${ type }` } position='middle'
                                                 formatter={ (value: number) => `${ charts[0][`${ tag }-${ type }`] } (${ formatNumber(100 * value / charts[0][type]) }%)` }/>
                                  }
                                  {
                                    (index === Object.keys(war.attackers).filter(tag => charts[0][`${ tag }-${ type }`] > 0).length - 1) && (charts[1][type] < charts[0][type]) &&
                                      <LabelList dataKey={ `${ tag }-${ type }` } position='top' fill='red'
                                                 formatter={ (value: number) => `+${ formatNumber(charts[0][type] - charts[1][type]) } (+${ formatNumber((1 - (charts[1][type] / charts[0][type])) * 100) }%)` }/>
                                  }
                                </Bar>
                              ))
                          }
                                ...
                                <Bar dataKey={ `${ tag }-${ type }` } stackId={ type } isAnimationActive={ false } key={ `bar-${ type }-${ tag }` }
                                     fill={ colorToHex(getCountry(save, tag).colors.countryColor) }>
                                  {
                                    charts[1][`${ tag }-${ type }`] / charts[1][type] > 0.03 &&
                                      <LabelList dataKey={ `${ tag }-${ type }` } position='middle'
                                                 formatter={ (value: number) => `${ charts[1][`${ tag }-${ type }`] } (${ formatNumber(100 * value / charts[1][type]) }%)` }/>
                                  }
                                  {
                                    (index === Object.keys(war.defenders).filter(tag => charts[1][`${ tag }-${ type }`] > 0).length - 1) && (charts[0][type] < charts[1][type]) &&
                                      <LabelList dataKey={ `${ tag }-${ type }` } position='top' fill='red'
                                                 formatter={ (value: number) => `+${ formatNumber(charts[1][type] - charts[0][type]) } (+${ formatNumber((1 - (charts[0][type] / charts[1][type])) * 100) }%)` }/>
                                  }
                                </Bar>
                              ))
                          }
