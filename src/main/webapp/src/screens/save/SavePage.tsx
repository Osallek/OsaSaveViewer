    ...
    if (timelapse || timelapseId || !save || maxTimelapse) {
      ...
      setTimelapseId(setInterval(() => {
        ...
        if (save && date) {
          ...
          handleDate(`${ d.getFullYear().toString().padStart(4, "0") }-${ (
            d.getMonth() + 1
          ).toString().padStart(
            2,
            "0"
          ...
          <>
            ...
            <div style={ { height: '100%', visibility: mapReady ? 'visible' : 'hidden', overflow: 'hidden' } }>
                    ...
                    getMapModes(save)
                      ...
                      .map((mm) => (
                        <Tooltip title={ intl.formatMessage({ id: `map.mod.${ mm }` }) } key={ `tooltip-${ mm }` }>
                          <Button
                            onClick={ () => handleMapMode(MapMode[mm]) }
              ...
              }
              {
                save &&
                <Chip
                  label={ intl.formatMessage({ id: 'common.graph' }) }
              ...
              }
              {
                ...
                <>
                  <Tooltip title={ intl.formatMessage({ id: 'common.export' }) }>
                    <IconButton
                      onClick={ () => setExportingModal(true) } disabled={ exporting } style={ {
                      position: 'absolute',
                      top: 92,
                      left: 52,
                      backgroundColor: theme.palette.primary.main,
                  ...
                  <ExportModal
                    open={ exportingModal } save={ save } onClose={ () => setExportingModal(false) } onExport={ exportImage }
                  />
                </>
              }
              {
              ...
              />
              {
                ...
                (
                  <Dialog
                    keepMounted fullScreen open={ statDialog } onClose={ () => setStatDialog(false) } closeAfterTransition
                  >
                    <SaveDialog save={ save } id={ id } onClose={ () => setStatDialog(false) } />
                  </Dialog>
                )
              }
            </div>
